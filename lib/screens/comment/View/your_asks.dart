...
class YourAsks extends StatefulWidget {
  @override
  State<YourAsks> createState() => _YourAsksState();
}

class _YourAsksState extends State<YourAsks> {
  List<Comment> datalistForYourAsks = List<Comment>();
  ...
  Future<void> importData(List commentsList) async {
    ...
    List<CommentF> commlis =
        await ApiServer.getAsker(askname: prefs.getString('name'));
    for (int i = 0; i < commlis.length; i++) {
      ...
      setState(() {
        datalistForYourAsks.add(comment);
        data.add(comment);
      });
    }
  }
            ...
            Padding(
              ...
              child: ElevatedButton(
                  onPressed: () {
                    setState(() {
                      List<Comment> datal = [];
                      datal.add(datalistForYourAsks[index]);
